...
if choice == "📝 新增租借紀錄":
    ...
    with col1:
        ...
        st.write("📝 **借用部門主管簽名確認：**")
        borrow_signature_confirm = st.checkbox("我確認以上資料正確無誤")
    ...
    with col2:
        ...
        st.markdown('<div class="signature-section">', unsafe_allow_html=True)
        st.write("📝 **借出部門主管簽名確認：**")
        loan_signature_confirm = st.checkbox("我確認以上資料正確無誤")
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with submit_col2:
        if st.button("📤 提交申請", use_container_width=True):
            if form_no and dept and item_code and description:
                if not borrow_signature_confirm or not loan_signature_confirm:
                    st.error("❌ 請確認雙方主管簽名")
                else:
                    new_record = {
                        "表單編號": form_no,
                        ...
                        "借用部門簽名確認": borrow_signature_confirm,
                        "借出部門簽名確認": loan_signature_confirm
                    }
